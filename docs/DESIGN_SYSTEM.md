...
> ⚠️ **Outdated reference (pre-pivot, апрель 2026).** Файл сохранён как
> исторический референс старого стиля. Полностью переписывается в
> фазе 5 (полный редизайн через Claude Design) — см. roadmap
> `.claude/plans/avito-project-b2b-drifting-haven.md`. До тех пор не
> использовать как источник правды для нового UI.
...
## Финансовые компоненты (owner)

**Путь:** `src/components/owner/finance/`

### FinanceDonut

Кольцевая диаграмма (Recharts PieChart) для визуализации финансовой структуры. Сегменты: Вложено, Прибыль, Расходы, Выплаты, Долги (условно). В центре — общий оборот. Легенда под кольцом.

```tsx
import { FinanceDonut } from "@/components/owner/finance";
<FinanceDonut
  segments={{ invested: 50000, profit: 12000, expenses: 8000, payouts: 5000, debts: 0 }}
/>;
```

### ProductROIBar

Двухфазный progress bar для отображения окупаемости товара:

- Фаза 1 (0–100%): окупаемость вложений (красный < 50%, оранжевый 50–99%)
- Фаза 2 (100%+): ROI (зелёный с glow-эффектом)

```tsx
import { ProductROIBar } from "@/components/owner/finance";
<ProductROIBar paybackPercent={156} totalInvested={20000} totalRevenue={31200} profit={11200} />;
```

### FinanceKPICards

4 KPI-карточки (Вложено, Выручка, Прибыль, ROI) в сетке 2×2 / 4×1. Цвета по контексту, отрицательная прибыль — красная.

### FinanceTabs

Переключатель табов: Товары, Расходы, Выплаты, Долги. Таб «Долги» появляется условно.
