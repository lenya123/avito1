-- confirm_pending_order_atomic v12: при confirm любого партнёрского заказа
-- автоматически записываем долг «партнёр должен мне» = partner_commission
-- из binding'а. Это новая модель (2026-05-05): любой партнёрский =
-- деньги партнёру → партнёр должен владельцу комиссию.
--
-- Если комиссия 0 или null — запись не создаём (нет смысла).
-- Долг привязан к order_id (после confirm pending уже удалён).
-- Видно в /owner/partners/<id> в блоке «💰 Партнёр должен мне».
...
BEGIN
  ...
  -- Комиссия: для partner-source — из binding, иначе NULL.
  ...
  -- Долг партнёра передо мной за этот заказ — комиссия с продажи.
  -- По новой модели любой партнёрский confirm = партнёр получил полную
  -- сумму, должен мне partner_commission. Если commission=0 (бесплатный
  -- товар для меня) — запись не создаём.
  IF v_pending.source_kind = 'partner'
