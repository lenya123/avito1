-- confirm_pending_order_atomic v14: для +ВАЙБ-партнёрских заказов
-- НЕ записываем partner_owner_debts сразу. Партнёр в этот момент ещё
-- НЕ получил деньги от клиента (это +ВАЙБ-долг — оплачивается позже
-- через wizard «💳 Оплатить долг»). Запись комиссии партнёра передо мной
-- должна появиться когда клиент реально оплатит партнёру и тот
-- подтвердит чек (см. миграцию следующего шага: триггер на
-- vibe_payment_orders + orders.is_paid).
...
BEGIN
  ...
  -- Записываем ТОЛЬКО для не-+ВАЙБ заказов: для +ВАЙБ-долговых партнёр
  -- ещё не получил деньги, поэтому и комиссии мне не должен. Запись
  -- появится позже — при подтверждении чека на vibe_payment.
  IF v_pending.source_kind = 'partner'
