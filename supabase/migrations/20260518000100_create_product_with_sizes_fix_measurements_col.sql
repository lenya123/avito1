-- ИСПРАВЛЕНИЕ: create_product_with_sizes ссылался на products.measurements,
-- которой НЕТ с 2026-04-25 (20260425000013 перенёс замеры на product_sizes,
-- product-level колонку убрал). Миграция 20260507000010 и 20260518000080
-- всё равно вставляли эту колонку → INSERT падал
-- («column "measurements" of relation "products" does not exist»).
-- Фактически RPC создания товара был сломан. Здесь:
--   • убираем products.measurements из INSERT (колонки нет);
--   • сохраняем замеры ПЕР-РАЗМЕР в product_sizes.measurements (§11.6).
