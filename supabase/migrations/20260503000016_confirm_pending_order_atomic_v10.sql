-- confirm_pending_order_atomic v10: source-aware DEC + копирование source_* snapshot в orders.
--
-- Изменения по сравнению с v9:
--   - Копирует source_kind / source_binding_id / source_partner_id / source_warehouse
--     из pending в orders.
--   - DEC: если source_kind='partner' → product_partner_size_stock по (binding_id, size_text);
--     иначе → product_sizes (как было).
--   - Если pending.is_vibe_debt=TRUE → orders.is_paid=FALSE, payment_method='deposit',
--     paid_at=NULL (как +ВАЙБ-flow). Карточка постится в группу клиентов другим вызовом.
--   - partner_commission_snapshot теперь берётся из binding.commission (если binding есть),
--     иначе из products.partner_commission (legacy fallback).
--   - Бамп payment_method_month_stats — без изменений (по payment_method_id IS NOT NULL).
...
BEGIN
  ...
  -- partner_commission_snapshot: из binding для partner-источника, иначе legacy products.
  ...
  ELSE
    SELECT partner_commission INTO v_partner_commission
      FROM public.products
     WHERE id = v_pending.product_id;
  END IF;

  -- Размер текстом — для DEC partner-source-stock и для триггера return_done.
  SELECT size INTO v_size_text
    FROM public.product_sizes
   WHERE id = v_pending.product_size_id;

  -- +ВАЙБ-долг: orders создаётся как is_paid=FALSE, payment_method='deposit'.
  IF COALESCE(v_pending.is_vibe_debt, FALSE) THEN
    v_is_paid        := FALSE;
    v_paid_at        := NULL;
  ...
  -- DEC current/reserved правильного источника.
  ...
  -- Бамп месячной статистики метода (только для owner+owner_warehouse — там стоит payment_method_id).
