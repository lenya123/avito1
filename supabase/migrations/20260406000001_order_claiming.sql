-- Система закрепления заказов за отправщиками.
-- Первый отправщик, который взаимодействует с заказом (печать/проблема/отправка),
-- закрепляет его за собой. Ночной сброс возвращает незавершённые в общий пул.

ALTER TABLE orders ADD COLUMN IF NOT EXISTS claimed_by UUID REFERENCES users(id);
...
-- Обновлённая функция: автоотвязка + подсчёт available по закреплённым заказам
...
BEGIN
  ...
  -- ═══ Автоотвязка: снимаем claim с заказов, закреплённых до начала текущего дня ═══
  UPDATE orders
  SET claimed_by = NULL, claimed_at = NULL
  WHERE claimed_by IS NOT NULL
    AND status IN ('awaiting_shipment', 'collecting', 'problem')
    AND claimed_at < p_date::TIMESTAMP;
  ...
  LOOP
    ...
    -- ═══ Orders available: закреплённые 1:1, незакреплённые делим поровну ═══

    -- 1. Закреплённые за этим отправщиком + отправленные им
    SELECT COUNT(*) INTO v_claimed
    FROM orders
    WHERE (
      -- Отправленные этим шиппером до cutoff
      (shipped_by = rec.id AND shipped_at >= p_date::TIMESTAMP AND shipped_at < v_cutoff)
      OR
      -- Закреплённые за ним и ещё не отправленные, до cutoff
      (claimed_by = rec.id AND status IN ('collecting', 'problem') AND claimed_at < v_cutoff)
    );

    -- 2. Незакреплённые (общий пул) до cutoff — делим между активными
    ...
    v_personal_available := v_claimed + GREATEST(CEIL(v_unclaimed::DECIMAL / v_active_shippers), 0);
    v_personal_available := GREATEST(v_personal_available, 1);
    ...
    -- Пересчитываем earnings с новой ставкой (S-кривая)
    DECLARE
