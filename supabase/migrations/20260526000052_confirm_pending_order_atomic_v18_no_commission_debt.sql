-- confirm_pending_order_atomic v18 (2026-05-26): убран INSERT в
-- partner_owner_debts(reason='partner_commission'). Canonical commission-долг
-- теперь — orders.partner_commission_snapshot + status='sent' + не paid_at
-- (см. §10.4). partner_owner_debts для commission больше не пишется и не
-- читается; таблица остаётся только для manual-корректировок владельца.
--
-- Остальное поведение v17 (dispatch_city snapshot) сохранено без изменений.
...
BEGIN
  ...
  -- v17→v18: убран INSERT INTO partner_owner_debts(reason='partner_commission').
  -- Долг по комиссиям теперь derived from orders, см. §10.4.
