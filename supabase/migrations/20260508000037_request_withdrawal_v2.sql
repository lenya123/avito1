-- v2: добавлен out_number — короткий номер запроса для DM владельцу.
CREATE FUNCTION public.request_withdrawal_atomic(
  p_customer_id UUID
) RETURNS TABLE (
  out_request_id UUID,
  out_number INTEGER,
...
BEGIN
  ...
  RETURNING id, withdrawal_number INTO v_request_id, v_number;
