-- replace_product_sizes v3: сохраняем initial_quantity (исторический baseline).
--
-- v2 при UPSERT перезаписывал initial_quantity = EXCLUDED (= введённое в
-- форме количество, т.к. buildSizeRowsForInsert шлёт initial=current=qty).
-- Из-за этого правка количества через форму товара сбрасывала «сколько
-- всего было заведено» и products.purchase_quantity.
--
-- v3: при обновлении существующего размера
--   current_quantity = введённое (как и было),
--   initial_quantity = GREATEST(старый initial, введённое)
-- → baseline никогда не теряется, растёт при доливе остатка, не падает
-- при коррекции вниз. Для НОВОГО размера initial = current = qty (какv2).
-- Сигнатура/return-type не меняются — CREATE OR REPLACE безопасен.
...
BEGIN
  ...
  -- initial_quantity сохраняем как максимум (baseline не теряется).
  ...
  ON CONFLICT (product_id, size) DO UPDATE
    SET initial_quantity = GREATEST(
          public.product_sizes.initial_quantity,
...
COMMENT ON FUNCTION public.replace_product_sizes IS
  'v3: UPSERT по (product_id, size); initial_quantity = GREATEST(старый, новый) — baseline не теряется. DELETE только orphan-размеров без ссылок из orders/pending_orders.';
