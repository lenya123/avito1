-- Фикс: chk_reservation_target требует чтобы было заполнено РОВНО одно из
-- product_size_id / product_id. Старая версия reserve_size_atomic писала
-- оба поля, что нарушало constraint и валило INSERT с code 23514.
--
-- Эта функция работает для товаров С размерами. Для товаров без размеров
-- (резерв на product_id напрямую) — отдельная RPC; здесь её скоупа нет.
...
BEGIN
  ...
  INSERT INTO size_reservations (product_size_id, session_id, expires_at)
  VALUES (p_product_size_id, p_session_id, v_new_expires)
