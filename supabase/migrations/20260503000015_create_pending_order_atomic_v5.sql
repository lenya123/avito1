-- create_pending_order_atomic v5: лестница источников + +ВАЙБ-флаг.
--
-- Параметры:
--   p_source_kind          'owner' | 'partner' (обязателен)
--   p_source_binding_id    UUID привязки (обязателен для partner)
--   p_size                 текст размера (обязателен для partner — DEC по binding+size)
--   p_is_vibe_debt         true → pending без TTL, без чека; ждёт «N да» от партнёра
--                          (или сразу confirm для owner-источника)
--
-- Snapshot в pending_orders:
--   - source_kind / source_binding_id / source_partner_id / source_warehouse
--   - is_vibe_debt
--   - applied_balance — только для не-+ВАЙБ И owner-collects (source_kind='owner' OR source_warehouse='owner')
...
BEGIN
  ...
  -- Применение баланса: только не-+ВАЙБ И owner собирает деньги.
  v_owner_collects := (p_source_kind = 'owner') OR (v_warehouse = 'owner');

  IF NOT p_is_vibe_debt AND v_owner_collects THEN
  ...
  -- Для +ВАЙБ TTL не выставляем — pending живёт до решения партнёра/клиента.
