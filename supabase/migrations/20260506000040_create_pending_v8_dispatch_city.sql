-- create_pending_order_atomic v8: считает и сохраняет dispatch_city.
-- Для partner_warehouse — из partners.warehouse_city, иначе из
-- products.location_city. Confirm_pending v17 копирует это в orders.
...
BEGIN
  ...
  -- Город отправки: partner_warehouse → partners.warehouse_city,
  -- иначе products.location_city.
  IF v_warehouse = 'partner' AND v_partner_id IS NOT NULL THEN
    SELECT warehouse_city INTO v_dispatch_city
      FROM public.partners WHERE id = v_partner_id;
  ELSE
    SELECT location_city INTO v_dispatch_city
