-- confirm_pending_order_atomic v13: копирует size из product_sizes
-- в orders.size при insert. Раньше колонка orders.size оставалась NULL для
-- новых заказов, и shipper-PWA думал что размер не выбран — всегда показывал
-- пикер «Выберите размер».
