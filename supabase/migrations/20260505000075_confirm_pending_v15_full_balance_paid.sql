-- confirm_pending_order_atomic v15: для +ВАЙБ-заказа с applied_balance,
-- который полностью покрыл цену → создаём orders как сразу оплаченный
-- (is_paid=TRUE, payment_method='balance'), не как долговый.
--
-- Это случается только для owner-source +ВАЙБ-заказов (для партнёрских
-- баланс не применяется по v7).
...
BEGIN
  ...
  -- Полностью оплачен с баланса = applied_balance >= client_price.
  ...
  IF COALESCE(v_pending.is_vibe_debt, FALSE) AND NOT v_fully_paid_balance THEN
    -- +ВАЙБ-долговый заказ: висит без оплаты до погашения через wizard.
    v_is_paid        := FALSE;
    v_paid_at        := NULL;
    v_payment_method := 'deposit';
  ELSIF v_fully_paid_balance THEN
    -- Полностью оплачен с баланса — оформляется сразу как оплаченный
    -- (даже если изначально был +ВАЙБ-долговый — баланс закрыл всё).
    v_is_paid        := TRUE;
    v_paid_at        := v_now;
  ...
  -- partner_owner_debts: только для не-+ВАЙБ партнёрских (когда партнёр
  -- получил деньги от клиента сразу). Для +ВАЙБ-долговых — запись пишется
  -- триггером trg_partner_debt_on_vibe_payment при is_paid=TRUE (когда
  -- клиент реально погасит долг).
  IF v_pending.source_kind = 'partner'
