-- Fix: update_shipper_scores падал с
--   column reference "shipper_id" is ambiguous
-- Причина: OUT-параметр `shipper_id` (RETURNS TABLE) совпадал по имени с
-- колонкой shipper_stats.shipper_id, которая используется в
--   INSERT INTO shipper_stats (...) VALUES (...) ON CONFLICT (shipper_id, date) ...
-- PostgreSQL не понимал, к чему относится `shipper_id` в ON CONFLICT.
--
-- Решение: директива `#variable_conflict use_column` — при коллизии имени
-- column vs variable выбираем column. OUT-параметр доступен через
-- присваивание `shipper_id := ...` (это всегда переменная). Тело функции
-- идентично 20260403000004_shipper_elo_score.sql.
...
BEGIN
  ...
  LOOP
    ...
    FROM shipper_stats ss
    WHERE ss.shipper_id = rec.id AND ss.orders_shipped > 0;
    ...
    BEGIN
      ...
      SELECT ss.orders_shipped INTO v_day_orders
      FROM shipper_stats ss
      WHERE ss.shipper_id = rec.id AND ss.date = p_date;

      IF v_day_orders > 0 THEN
        UPDATE shipper_stats ss SET
