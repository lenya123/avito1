-- reconcile_product_stock: те же тело/семантика, но nullable-параметры
-- получают DEFAULT NULL. Цель — корректная типизация: supabase gen types
-- помечает параметры с DEFAULT как необязательные, и в TS-вызове их можно
-- просто не передавать вместо передачи `null` (который генератор типов
-- иначе не допускает для INT/JSONB-аргументов). Семантика не меняется:
-- функция и так трактует NULL как «нет этой части инвентаризации».
--
-- CREATE OR REPLACE с добавлением DEFAULT параметрам допустимо (дефолты
-- не входят в сигнатуру функции, return-type не меняется).

CREATE OR REPLACE FUNCTION public.reconcile_product_stock(
  p_product_id    UUID,
  p_sizes         JSONB DEFAULT NULL,
  p_no_size_count INT   DEFAULT NULL,
