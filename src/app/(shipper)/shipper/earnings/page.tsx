...
function getCurrentMonthName(): string {
  return new Date().toLocaleDateString("ru-RU", { month: "long", timeZone: MOSCOW_TZ });
}
...
export default function ShipperEarningsPage() {
  ...
  const paymentMode = stats?.paymentMode ?? "pendulum";
    ...
    <div className="min-h-screen">
      ...
      >
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          <h1 className="text-xl font-bold text-white">Деньги</h1>
          <Toggle checked={!isFixed} disabled size="sm" label={isFixed ? "Фикс" : "Маятник"} />
        </div>
      </header>
            ...
            >
              ...
              <div className="relative">
                ...
                <p className="text-sm text-white/40 mt-3">
                  Начислено и ещё не выплачено. Владелец переводит вручную и фиксирует выплату.
                </p>
              </div>
            </motion.div>

            {/* ─── Маятник (только dynamic) ─────────────────── */}
            {!isFixed && efficiency && <PendulumBar data={efficiency} />}
