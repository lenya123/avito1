...
import { AvitoSessionConnect } from "@/components/owner/avito/avito-session-connect";
import { AiAgentCard } from "@/components/owner/avito/ai-agent-card";
import {
  DashboardHeader,
  DashboardOverview,
  ...
  DashboardOperations,
  DashboardChats,
} from "@/components/owner/avito/dashboard";
...
export default function AvitoDashboardPage() {
  ...
  // Хуки данных вызываем (триггерят загрузку + кормят секции через кэш), но их
  // loading НЕ гейтит всю страницу — иначе при ошибках avito-данных (нет
  // подключённого аккаунта) страница вечно висит в «Загрузка...».
  useAvitoOverview();
  useAvitoItems(1, 6);
  useAvitoChats(1, 4);
  useAvitoReviews(0, 3);
  ...
  // Спиннер гейтим ТОЛЬКО на проверке сессии. Секции дашборда (в т.ч.
  // «Объявления» с кнопкой «+ Создать») рендерятся сразу и сами показывают
  // свой скелетон/пустое состояние.
