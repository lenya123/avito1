...
const connectSchema = z.object({
  ...
  accountIndex: z.number().int().min(1).max(99).optional(),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { login, password, accountIndex = 1, displayName } = parsed.data;
    const supabase = createServiceClient();

    // Standalone: subscription/vibe-гейт снят. Лимит аккаунтов также убран —
    // подключаем любое количество магазинов без ограничений.
    ...
    // display_name пишем только если задан (пустую строку → NULL),
    // иначе старое значение сохранится при повторном connect'е.
    ...
    if (displayNameClean) upsertPayload.display_name = displayNameClean;

    const { error: upsertError } = await supabase
      .from("avito_browser_sessions")
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      .upsert(upsertPayload as any, { onConflict: "user_id,account_index" });
...
// DELETE — полностью удалить сессию аккаунта (UI крестик).
// После этого аккаунт исчезает из switcher; чтобы пользоваться снова — подключить заново.
