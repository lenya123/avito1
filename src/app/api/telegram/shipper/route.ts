/**
 * Webhook endpoint для бота отправщиков
 *
 * POST /api/telegram/shipper
 */

import { NextRequest, NextResponse } from "next/server";
import { webhookCallback } from "grammy";
import { createShipperBot } from "@/lib/telegram/bots/shipper-bot";

// Lazy-инициализация бота
let bot: ReturnType<typeof createShipperBot> | null = null;
let handleUpdate: ((request: Request) => Promise<Response>) | null = null;

function getBot() {
  if (!bot) {
    bot = createShipperBot();
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("Shipper bot webhook error:", error);
