...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: todayStats } = await supabase
      .from("shipper_stats")
      .select("shipper_id, orders_shipped, returns_collected, orders_available")
    ...
    const todayStatsMap: Record<
      string,
      { shipped: number; returned: number; ordersAvailable: number }
    > = {};
    ...
    todayStats?.forEach((stat) => {
      if (!todayStatsMap[stat.shipper_id]) {
        todayStatsMap[stat.shipper_id] = { shipped: 0, returned: 0, ordersAvailable: 0 };
      }
      ...
      todayStatsMap[stat.shipper_id].ordersAvailable += stat.orders_available || 0;
    });
    ...
    const shippersFormatted = shippers?.map((shipper) => ({
      ...
      today: todayStatsMap[shipper.id] || { shipped: 0, returned: 0, ordersAvailable: 0 },
...
const createShipperSchema = z.object({
  ...
  telegramUsername: z.string().min(1, "Telegram обязателен"),
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Проверяем, нет ли уже отправщика с таким telegram_username
    ...
    if (existingShipper) {
      return NextResponse.json(
        { error: "Отправщик с таким Telegram уже существует" },
        { status: 400 }
      );
    }

    // Генерируем site_key (64 hex символа)
