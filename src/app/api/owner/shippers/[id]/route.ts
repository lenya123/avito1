...
const updateShipperSchema = z
  ...
  .refine(
    (data) => {
      if (data.workHourStart !== undefined && data.workHourEnd !== undefined) {
        return data.workHourEnd > data.workHourStart;
      }
      return true;
    },
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (data.workHourStart !== undefined) updateData.work_hour_start = data.workHourStart;
    if (data.workHourEnd !== undefined) updateData.work_hour_end = data.workHourEnd;
...
export async function DELETE(
  ...
  try {
    ...
    // Проверяем нет ли активных заказов (канон §4.2: paid/collecting — до отправки)
