...
import { notifyShippersOrderUrgent } from "@/lib/telegram/notifications";
import { z } from "zod";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (product_size_id) {
      ...
      // Доступно = остаток − мягкий резерв (как в каталоге/визарде/AI;
      // §4.1 soft-reservation). Раньше тут был только current_quantity →
      // можно было создать заказ поверх зарезервированного (oversell).
      const available = (productSize.current_quantity ?? 0) - (productSize.reserved_quantity ?? 0);
      if (available <= 0) {
    ...
    // 4. Create order.
    // Ручной заказ от отправщика не привязан к клиенту (customer_id NULL).
    // В customer_name_snapshot/customer_tg_username_snapshot это проявится как NULL —
    // shipper-UI рендерит "—" для таких заказов.
    ...
    const { data: order, error: orderError } = await supabase
      ...
      .insert({
        ...
        size: size || null,
        delivery_service,
        send_by: deadlineDate.toISOString().split("T")[0],
        // pickup_by NOT NULL (миграция 20260526000041) — для прямого заказа
        // ставим тот же дефолт что для send_by; реально pickup_by актуален
        // только при оформлении возврата.
        pickup_by: deadlineDate.toISOString().split("T")[0],
        purchase_price: product.purchase_price ?? 0,
        client_price: product.drop_price ?? 0,
        tracking_number: trackingNumber,
        status: "paid",
        source: "manual",
        status_history: [{ status: "paid", timestamp: new Date().toISOString() }],
        is_paid: true,
    ...
    notifyShippersOrderUrgent(order.id).catch((e) =>
      console.error("notifyShippersOrderUrgent failed:", e)
    );
