...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // 5. Инвентаризация (физпересчёт отправщика) — АТОМАРНАЯ сверка через
    // RPC reconcile_product_stock: событие в stock_reconciliations +
    // авто-сверка current := факт в ОДНОЙ транзакции, для размерных и
    // One Size единообразно. Раньше было инлайном по шагам без транзакции
    // (риск рассинхрона аудит-журнала) + ветка One Size вообще не писала
    // недостачу. Канон §11.4.
    const hasSizedCount = !!data.actual_sizes && data.actual_sizes.length > 0;
    const hasNoSizeCount = data.actual_quantity !== undefined && !data.actual_sizes;
    if (hasSizedCount || hasNoSizeCount) {
      const { error: reconErr } = await supabase.rpc("reconcile_product_stock", {
        p_product_id: productId,
        p_by: session.userId,
        ...(hasSizedCount
          ? {
              p_sizes: data.actual_sizes!.map((s) => ({
                size_id: s.size_id,
                counted: s.actual_quantity,
              })),
            }
          : {}),
        ...(hasNoSizeCount ? { p_no_size_count: data.actual_quantity! } : {}),
      });
      if (reconErr) {
        console.error("Reconciliation RPC error:", reconErr);
        return NextResponse.json({ error: "Ошибка фиксации сверки" }, { status: 500 });
      }
    }
