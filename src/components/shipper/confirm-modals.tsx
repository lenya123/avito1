...
interface ConfirmModalsProps {
  ...
  problemOpen: boolean;
  onProblemClose: () => void;
  onMarkProblem: (type: "out_of_stock" | "bad_barcode", scope?: "single" | "all") => void;
}
          ...
          {selectedCount === 1 ? (
            <>
              <Button
                ...
                onClick={() => onMarkProblem("out_of_stock", "single")}
              ...
              <Button
                ...
                size="lg"
                className="w-full justify-start"
                onClick={() => onMarkProblem("out_of_stock", "all")}
                isLoading={batchPending}
              >
                <svg
                  className="w-5 h-5 mr-3 text-accent-red shrink-0"
                ...
                <span className="text-left">
                  Нет товара совсем
                  <span className="block text-xs text-white/40 font-normal">
                    на все заказы по этому размеру
                  </span>
                </span>
              </Button>
            </>
