...
type GroupMode = "priority" | "product";
...
function groupByProduct(orders: ShipperOrder[]): { label: string; orders: ShipperOrder[] }[] {
  ...
  return Array.from(map.entries())
    ...
    .map(([label, groupOrders]) => ({
      ...
      // Единый компаратор размеров (@/utils/sizes) — было localeCompare,
      // ломало порядок числовых/«M (31)» относительно остальных экранов.
      orders: groupOrders.sort((a, b) => compareSizes(a.size || "", b.size || "")),
    }));
}
