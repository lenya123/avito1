...
interface BottomActionBarProps {
  ...
  /** Ship-таб: открыть окно печати стикеров (печать опциональна, §4.2). */
  ...
  return (
    ...
    >
      <div className="max-w-4xl mx-auto space-y-2">
        ...
        {activeFilter === "ship" && (
          <>
            {/* Печать опциональна (§4.2): «Сдал в ПВЗ» доступен всегда для
                выбранных collecting-заказов. Печать стикера не блокирует. */}
            <div className="flex gap-3">
              <Button
                variant="primary"
                size="lg"
            ...
            {/* Печать стикеров — по желанию, не обязательный этап. */}
            <Button
              variant="secondary"
              size="lg"
              ...
              isLoading={batchPending}
            >
              🖨 Напечатать стикеры ({selectedCount})
            </Button>
          </>
        )}
