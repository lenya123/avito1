        ...
        >
          <AnimatePresence mode="popLayout">
            {pickupPoints.map((point) => (
              ...
              >
                {/* role=button, не <button>: внутри вложена кнопка «Удалить»,
                    а button-в-button — невалидный HTML (hydration error). */}
                <div
                  role="button"
                  tabIndex={deletingId === point.id ? -1 : 0}
                  aria-disabled={deletingId === point.id}
                  onClick={() => {
                    if (deletingId === point.id) return;
