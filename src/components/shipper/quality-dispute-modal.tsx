...
export function QualityDisputeModal({
  ...
  const handleSubmit = async () => {
    if (photos.length < 3) {
      toast.error("Нужно минимум 3 фотографии");
      return;
    }
    if (reason.trim().length < 5) {
      toast.error("Опиши проблему (минимум 5 символов)");
      return;
    }
    await onSubmit(photos, reason.trim());
    setPhotos([]);
    setReason("");
  };
        ...
        <Modal isOpen={isOpen} onClose={handleClose} title="Проблема с качеством">
          <div className="space-y-4">
            <p className="text-white/70 text-sm">
              Заказ №{orderNumber}. Сфотографируй повреждения и опиши проблему — клиенту уйдёт
              уведомление с фото и инструкцией обратиться в поддержку Авито. Деньги не возвращаются.
            </p>
            <p className="text-white/50 text-xs">
              Минимум 3 фото и описание — обязательно. Авито-поддержка разберёт случай.
            </p>
            ...
            {photos.length < 5 && (
              ...
              >
                ...
                <span className="text-sm">
                  Добавить фото ({photos.length}/5
                  {photos.length < 3 ? `, минимум ${3 - photos.length}` : ""})
                </span>
              </button>
            )}
            ...
            <Input
              ...
              placeholder="Например: товар повреждён, пятна на ткани, не та модель..."
            />
            ...
            <div className="flex gap-3">
              ...
              <Button
                ...
                isLoading={isLoading}
                disabled={photos.length < 3 || reason.trim().length < 5}
              >
                Отправить спор
              </Button>
            </div>
