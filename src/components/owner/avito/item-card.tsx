...
interface AvitoItemCardProps {
  ...
  onToggleClick?: () => void;
  /** Снять с публикации (для активных). Внутри вызывает DELETE ?mode=stop. */
  onStopClick?: () => void;
  /** Удалить из архива (для снятых). Внутри вызывает DELETE ?mode=delete. */
  onDeleteClick?: () => void;
}
  ...
  return (
    ...
    >
      ...
      <div className="aspect-[4/3] bg-white/5 relative">
        ...
        {/* ТЗ §3.2: бейдж «не привязано к товару» — без mapping мини-AI
            не сможет узнать какие размеры спрашивать у покупателя. */}
        {!isLinked && (
          <Badge
            variant="error"
            size="sm"
      ...
      <div className="p-3">
        ...
        {/* Управление */}
        {(onLinkClick || onPriceEditClick || onToggleClick || onStopClick || onDeleteClick) && (
