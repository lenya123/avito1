...
interface AvitoSessionConnectProps {
  /** Если передан — компонент работает с указанным аккаунтом
   *  (для модалки подключения новых магазинов). Без props'а — текущий активный. */
  accountIndex?: number;
}

export function AvitoSessionConnect({ accountIndex }: AvitoSessionConnectProps = {}) {
  const { data: session, isLoading } = useAvitoSession(accountIndex);
  const connectMutation = useConnectAvitoSession(accountIndex);
  const disconnectMutation = useDisconnectAvitoSession(accountIndex);
  const submitSmsMutation = useSubmitSmsCode(accountIndex);
      ...
      <Card padding="sm">
        ...
        {showForm && (
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-3 mt-3">
            <Input
              placeholder="Название аккаунта (необязательно)"
