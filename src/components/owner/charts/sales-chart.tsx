...
export type ChartDataPoint = {
  ...
  profit: number;
  invested: number;
  /** Σ расходов за день — опционально (только Финансы кормит эту метрику;
   *  Заказы/Отправщики данные без неё, переключатель её не покажет). */
  expenses?: number;
};

export type ChartMetric = "orders" | "revenue" | "profit" | "expenses";
...
export interface SalesChartProps {
  ...
  metricLabels?: Partial<Record<ChartMetric, string>>;
  visibleMetrics?: ChartMetric[];
  /** Hide the header (title + metric toggle + selected value). Useful when parent provides its own. */
  hideHeader?: boolean;
  /** Custom title (replaces "Динамика") */
  title?: string;
  /** Render custom content in the header's right side (replaces metric toggles) */
  headerRight?: React.ReactNode;
  /** Render custom content below header (e.g. date pickers) */
  headerBelow?: React.ReactNode;
  /** Custom selected bar display. Return null to use default. */
  renderSelected?: (bar: {
    value: number;
    label: string;
    date: string;
...
export function SalesChart({
  ...
  const [metric, setMetric] = useState<ChartMetric>(
    // Если задан visibleMetrics — дефолт = первая из перечисленных (иначе
    // переключатель показывал бы кнопки revenue/profit/expenses, а активной
    // была бы скрытая «orders»). Без visibleMetrics — дефолт "orders".
    visibleMetrics && visibleMetrics.length > 0 ? visibleMetrics[0] : "orders"
  );
  ...
  const chartInfo = useMemo(() => {
    ...
    // `expenses` опционален — отсутствие = 0 (страница без этой метрики).
    const values = displayData.map((d) => d[metric] ?? 0);
      ...
      {!hideHeader && (
        <div className="p-4 pb-2">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold text-white">{title ?? "Динамика"}</h3>
            {headerRight ?? (
              <div className="flex gap-1">
                {(visibleMetrics ?? (Object.keys(METRIC_CONFIG) as ChartMetric[])).map((m) => (
                  <button
                    key={m}
                    onClick={() => {
                      setMetric(m);
                  ...
                  >
                    {metricLabels?.[m] ?? METRIC_CONFIG[m].label}
                  </button>
                ))}
              </div>
            )}
          ...
          <div className="h-8 flex items-center">
            {selectedIndex !== null && chartInfo.bars[selectedIndex] ? (
              renderSelected ? (
                renderSelected({
                  value: chartInfo.bars[selectedIndex].value,
                  label: chartInfo.bars[selectedIndex].label,
                  date: chartInfo.bars[selectedIndex].date,
                  orders: data?.[selectedIndex]?.orders ?? 0,
                  revenue: data?.[selectedIndex]?.revenue ?? 0,
                  profit: data?.[selectedIndex]?.profit ?? 0,
                })
              ) : (
                <motion.div
                ...
                >
                  ...
                  <span className="text-sm text-white/40">
                    {chartInfo.bars[selectedIndex].label}
                  </span>
                </motion.div>
              )
            ) : (
