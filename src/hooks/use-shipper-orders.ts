...
export function useShipperHistoryOrders(search: string, offset: number, limit = 50) {
  return useQuery({
    ...
    queryFn: async () => {
      const params = new URLSearchParams();
      // Канон §4.2 (после legacy-cleanup 2026-05-15): sent/return_done/trash.
      // Старые имена (completed/return_completed/disposed) ломали isHistoryQuery
      // → сортировка падала на send_by ASC и пагинация не применялась.
      params.set("statuses", "sent,return_done,cancelled,trash");
