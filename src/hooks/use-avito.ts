...
interface AvitoAccountInfo {
  ...
  /** Кастомное имя от владельца (вводится при подключении). */
  displayName: string | null;
  /** Имя магазина из Avito API (avito_browser_sessions.shop_name). */
  shopName: string | null;
  /** Логин/телефон HTTP-login (если использовался). */
...
export function useUpdateItemPrice() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      // Полная инвалидация — items, overview-KPI, AI-агент, заказы.
      queryClient.invalidateQueries({ queryKey: ["avito"] });
    },
  });
}
...
export function useAvitoSession(overrideAccountIndex?: number) {
  const activeAccountIndex = useActiveAccountIndex();
  const accountIndex = overrideAccountIndex ?? activeAccountIndex;
  return useQuery({
...
export function useConnectAvitoSession(overrideAccountIndex?: number) {
  const queryClient = useQueryClient();
  const activeAccountIndex = useActiveAccountIndex();
  const accountIndex = overrideAccountIndex ?? activeAccountIndex;
  return useMutation({
    mutationFn: (data: { login: string; password: string; displayName?: string }) =>
      connectAvitoSession({ ...data, accountIndex }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["avito"] }),
  });
}

export function useDisconnectAvitoSession(overrideAccountIndex?: number) {
  const queryClient = useQueryClient();
  const activeAccountIndex = useActiveAccountIndex();
  const accountIndex = overrideAccountIndex ?? activeAccountIndex;
  return useMutation({
...
export function useSubmitSmsCode(overrideAccountIndex?: number) {
  const queryClient = useQueryClient();
  const activeAccountIndex = useActiveAccountIndex();
  const accountIndex = overrideAccountIndex ?? activeAccountIndex;
  return useMutation({
...
export function useCreatePost() {
  ...
  return useMutation({
    mutationFn: async (body: {
      ...
      saveDescriptionToProduct?: boolean;
    }) => {
...
export type PresetKind = "preview" | "cover" | "photoset" | "ai-preview" | "photozone" | "personality";
...
export interface PhotosetSetRow {
  set_key: string;
  title: string | null;
  photo_count: number;
  usage_count: number;
...
export function usePresets() {
  return useQuery<{ presets: PresetRow[]; sets: PhotosetSetRow[] }>({
...
// ─── Создать объявление: список только своих товаров ───
export interface AvitoListingProduct {
  id: string;
  name: string;
...
export function useAvitoListingProducts(search: string, enabled: boolean = true) {
  return useQuery<{ products: AvitoListingProduct[] }>({
    queryKey: ["avito", "listing-products", search],
    queryFn: async () => {
      const res = await fetch(
        `/api/avito/listings/products?search=${encodeURIComponent(search)}`
      );
...
// ─── Живой фотосет (датасет) товара: фото для выбора исходников AI ───
export function useProductDataset(productId: string | null, enabled: boolean = true) {
  return useQuery<{ photos: { id: string; url: string }[] }>({
    queryKey: ["avito", "product-dataset", productId],
    queryFn: async () => {
      const res = await fetch(`/api/avito/listings/dataset?productId=${productId}`);
      if (!res.ok) throw new Error("Ошибка загрузки датасета");
      return res.json();
    },
...
// ─── AI-генерация фото: остаток дневной квоты + запуск ───
export type AiPhotoCategory = "normal" | "photozone" | "personality";

export interface AiPhotoQuota {
  caps: Record<AiPhotoCategory, number>;
  used: Record<AiPhotoCategory, number>;
  remaining: Record<AiPhotoCategory, number>;
  /** Всего генераций по товару (монотонно растёт) — сигнал «фото готово/пришло в TG». */
  generatedCount: number;
  /** Готовых обложек-кандидатов для товара (preview/cover/ai-preview) — банк слота 1. */
  coverCount: number;
}

export function useAiPhotoQuota(productId: string | null, enabled: boolean = true) {
  return useQuery<AiPhotoQuota>({
    queryKey: ["avito", "ai-photo-quota", productId],
    queryFn: async () => {
      const res = await fetch(`/api/avito/autopost/ai-photo?productId=${productId}`);
      if (!res.ok) throw new Error("Ошибка загрузки квоты");
      return res.json();
    },
    enabled: enabled && !!productId,
    staleTime: 3 * 1000,
    // Слот списывается воркером (claim_ai_gen_slot) и generatedCount растёт по факту
    // готовности фото — полинг подтягивает реальное состояние, пока модалка открыта.
    refetchInterval: enabled && !!productId ? 5 * 1000 : false,
  });
}

export function useGenerateAiPhoto() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (body: {
      productId: string;
      ...
      const res = await fetch("/api/avito/autopost/ai-photo", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
      });
    ...
    // Без optimistic-апдейта: счётчик/блокировку держит модалка через состояние
    // «генерируется» + max-формулу (раньше optimistic перетирался рефетчем — «отскок»).
    onSuccess: (_data, vars) => {
      queryClient.invalidateQueries({ queryKey: ["avito", "ai-photo-quota", vars.productId] });
    },
  });
}
