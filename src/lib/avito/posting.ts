...
export interface PostListingInput {
  ...
  city: string;
  metro: string | null;
  /** Адрес «улица, дом» для городов без метро-логики (метро при этом null). */
  address?: string | null;
...
export async function postListing(
  ...
  try {
    ...
    // 4. Локация: метро (Москва/Питер) или адрес (другие города)
    ...
    if (input.metro) {
      ...
      if (metroOk) {
        ...
        await clickFirst(page, SUGGEST_SELECTORS).catch(() => false);
      }
    } else if (input.address) {
      // STUB: verify selectors against live Avito — поле адреса для не-Москвы/Питера
      const addrOk = await fillByCandidates(
        page,
        [
          'input[data-marker="address/address/input"]',
          'input[placeholder*="дрес"]',
          'input[name="address"]',
        ],
        input.address
      );
