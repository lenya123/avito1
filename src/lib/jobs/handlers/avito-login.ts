...
import { fetchAvitoApiKey } from "@/lib/avito/web-client";
import type { AvitoLoginJobData } from "../queues";

/**
 * ТЗ §15.9: после успешного login извлекаем per-user apiKey из HTML
 * orders-страницы и сохраняем в avito_browser_sessions.api_key.
 * Это нужно для BeduinUI endpoints (/api/2/profile/order, /api/2/order-log)
 * — они возвращают buyer.name, комиссию, channelId, полный timeline.
 * Best-effort: ошибка извлечения не валит login.
 */
async function tryExtractAndSaveApiKey(
  ...
  try {
    const apiKey = await fetchAvitoApiKey({
      cookies,
      userAgent,
      proxyUrl,
    });
    if (apiKey) {
      ...
      console.log(`[avito-login] ✅ api_key extracted (${apiKey.slice(0, 8)}…)`);
    } else {
      console.warn(`[avito-login] api_key extraction failed (regex no match)`);
    }
  } catch (e) {
    console.warn(`[avito-login] api_key extraction error:`, (e as Error).message);
  }
}
  ...
  if (session.sms_code && smsContextStore.has(key)) {
    ...
    try {
      ...
      if (r.status === "success" && r.cookies) {
        ...
        await tryExtractAndSaveApiKey(
          supabase,
          userId,
          accountIndex,
          r.cookies,
          ctx.userAgent,
          session.proxy_url as string | null
        );
        return;
      }
