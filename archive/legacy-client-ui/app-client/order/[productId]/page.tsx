...
import { usePickupPointsForCity } from "@/hooks/use-location-pickup-points";
...
export default function OrderPage() {
  ...
  // Pickup points for the product's city
  const { data: allPickupPoints } = usePickupPointsForCity(product?.location_city);
  const filteredPickupPoints = useMemo(
    () => (allPickupPoints || []).filter((p) => p.deliveryService === deliveryService),
    [allPickupPoints, deliveryService]
  );

  // Reset pickup point when delivery service changes
  useEffect(() => {
                ...
                <div className="mt-4">
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                    {filteredPickupPoints.map((point) => {
                      const isSelected = pickupPointId === point.id;
                      return (
                        <motion.button
                          key={point.id}
                          type="button"
                          ...
                          className={cn(
                            ...
                            isSelected
                              ...
                              : "bg-white/[0.06] border border-glass-subtle hover:bg-white/[0.1]"
                          )}
                        >
                          <p
                            className={cn(
