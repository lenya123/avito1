...
import { ORDER_STATUS_LABELS, type OrderStatus, useOrderedProducts } from "@/hooks/use-orders";
import { Button, DatePicker, ProductPicker } from "@/components/ui";
...
export function OrderFilters({ filters, onChange, className }: OrderFiltersProps) {
  ...
  const { data: orderedProducts, isLoading: productsLoading } = useOrderedProducts();

  const pickerProducts: ProductPickerItem[] = (orderedProducts || []).map((p) => ({
    id: p.id,
    name: p.name,
    brand: p.brand,
    photoUrl: p.photoUrl,
  ...
  const handleReset = useCallback(() => {
    onChange({ status: "", dateFrom: "", dateTo: "", period: "all", productId: undefined });
  }, [onChange]);

  const hasActiveFilters =
    filters.status || (filters.period && filters.period !== "all") || filters.productId;
