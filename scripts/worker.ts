...
import {
  ...
  scheduleSweep,
  scheduleFraudDetectorsDaily,
  scheduleDailyShipperCleanup,
  scheduleDirectorReceiptsDigest,
  schedulePartnerReceiptsDigest,
  scheduleWithdrawalRequestsDigest,
  scheduleShipperPoolDigest,
  scheduleExpiredOrdersMorningDigest,
  scheduleAvitoReloginCheck,
  scheduleProxyHealthCheck,
  scheduleAvitoBalanceSync,
  scheduleNightlyCoverGeneration,
  scheduleAvitoProcessAwaitingSize,
} from "../src/lib/jobs/queues";
import { sweepExpiredOrders } from "../src/lib/jobs/sweep-expired-orders";
...
const avitoEnabled = process.env.AVITO_ENABLED === "true";

void (async () => {
  try {
    // Sweep при старте — подчищает пропущенное
    await sweepExpiredOrders();

    const repeatable: Array<Promise<unknown>> = [
      scheduleSweep(),
      scheduleFraudDetectorsDaily(),
      scheduleDailyShipperCleanup(),
      scheduleDirectorReceiptsDigest(),
      ...
      scheduleShipperPoolDigest(),
      scheduleExpiredOrdersMorningDigest(),
      scheduleNightlyCoverGeneration(), // 03:00 МСК — автогенерация AI-обложек
    ];
    ...
    if (avitoEnabled) {
      ...
      console.log(
        "⏭  AVITO_ENABLED!=true — Avito-очереди пропущены (Этап 9 продолжит интеграцию)."
      );
    }

    await Promise.all(repeatable);
    console.log("✅ Repeatable jobs registered (including daily sweep at 00:00 MSK).");
  } catch (err) {
